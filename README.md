...
├── web/                        static replay demo deployed to Vercel
│   ├── index.html              one page; see web/README.md for what it can't do
│   ├── app.js                  rendering + 3Dmol viewer
...
`web/` is a static replay of the recorded run, and it is what deploys to Vercel.
`vercel.json` sets `outputDirectory` to `web/` with a no-op build; `.vercelignore`
keeps the Python project out of the upload so zero-config detection does not try
to build a serverless function from `requirements.txt`.
...
**The live pipeline does not run on Vercel, and no configuration makes it.**
...
Gemma is served by a local Ollama process that has no serverless equivalent. The
deployed page says this on its face and labels every number as either recomputed
at build time from committed structures or transcribed from the recorded run.
See **[web/README.md](web/README.md)** for the full provenance rules.

To host the interactive app, use somewhere that can run a persistent process with
GPU or large-CPU memory — Hugging Face Spaces, Render, Fly.io — and expect to
supply the checkpoint and an Ollama endpoint yourself.
