...
├── api/fold.mjs                structure-prediction proxy (the only server code)
├── web/                        the demo deployed to Vercel
│   ├── index.html              one page; see web/README.md for what it can't do
│   ├── app.js                  rendering, viewers, policy editor, repair pipeline
...
│   ├── dev_server.py           serves web/ + /api/fold locally
...
python scripts/dev_server.py       # preview at http://localhost:8765
```

Use `dev_server.py` rather than `http.server`: the repair section needs the
`/api/fold` endpoint that Vercel provides in production, and the plain static
...
- **Repair your own protein.** Paste a sequence — protein, or DNA which is
  translated first — and it is folded, scored residue by residue by ESM-2 running
  in the browser, repaired by the same interpreter, and folded again. You get the
  predicted structure as given, what should change and why, the before/after
  comparison, and the corrected sequence as FASTA. ESM-2 is loaded as fp16
  because int8 error is large enough to reorder the position ranking.

**The 8.4 GB ESMFold checkpoint still does not run on Vercel** — `api/fold.mjs`
is a dependency-free proxy to Meta's hosted ESMFold v1, because ESM Atlas's CORS
preflight blocks the browser from calling it directly. That service is free and
intermittently unavailable, so the proxy retries and a fold failure degrades the
page rather than breaking it.

What a pasted sequence still cannot get is a **verdict**. There is no reference
structure for a protein you supply, so there is no hidden verifier and nothing is
scored against truth. The page reports only what is computable — sequence
likelihood, the model's own confidence, and how far the predicted fold moved —
and says plainly that a repair improving all three is a hypothesis, not a
verified fix. See **[web/README.md](web/README.md)** for the provenance rules and
